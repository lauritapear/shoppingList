...
describe("Items Service", () => {
  ...
  beforeAll(async (done) => {
    ...
    mongoDatabase = new DatabaseManager(uri, dbName);
  ...
  it("gets all items", async (done) => {
    const pageSize = 2;
    const pageNumber = 0;
    const items = await ItemService.getAll(pageNumber, pageSize);
    items[0].paginatedResults.forEach((item: IItem, index: number) => {
      expect(item.name).toEqual(testItems[index].name);
    });
    expect(items[0].totalCount[0].count).toEqual(testItems.length);
    done();
  });
  ...
  it("creates an item", async (done) => {
    ...
    expect(itemFromDb && itemFromDb.description).toEqual(
  ...
  it("updates an item ", async (done) => {
    ...
    expect(updatedItem && updatedItem.description).toEqual(
      testItems[0].description
    );
    done();
  });
